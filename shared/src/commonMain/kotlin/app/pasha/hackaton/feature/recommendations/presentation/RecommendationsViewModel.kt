...
class RecommendationsViewModel(
...
) : ViewModel(), Stateful<RecommendationState> by statefulViewModel(RecommendationState.Loading), KoinComponent {
    ...
    private fun loadData() {
        viewModelScope.launch {
            updateState { RecommendationState.Ready() }
        }
    }
    ...
    private fun observeUser() {
        viewModelScope.launch {
            userRepository.userName.collect { name ->
                updateState { state ->
                    when (state) {
                        RecommendationState.Loading -> RecommendationState.Loading
                        is RecommendationState.Ready -> state.copy(userName = name)
                    }
                }
            }
        }
    ...
    fun onApply(id: String) {
        updateState { state ->
            when (state) {
                RecommendationState.Loading -> state
                is RecommendationState.Ready -> {
                    val recommendation = state.recommendations.find { it.id == id }
                    state.copy(selectedRecommendation = recommendation)
                }
            }
        }
    }
    ...
    fun onConfirmApply() {
        updateState { state ->
            when (state) {
                RecommendationState.Loading -> state
                is RecommendationState.Ready -> state.copy(selectedRecommendation = null)
            }
        }
    }
